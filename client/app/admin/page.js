...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchForTab = async () => {
      ...
      try {
        ...
        if (active === "Requests" && requests.length === 0)
          await loadRequests();
  ...
  const handleBanToggle = async (user) => {
    try {
      await adminSetUserBan(user._id, !user.isBanned);
      await loadUsers();
    } catch (err) {
  ...
  const handleDeletePost = async (post) => {
    ...
    try {
      await adminDeletePost(post._id);
      await loadPosts();
    } catch (err) {
  ...
  const handleSpamToggle = async (post) => {
    try {
      await adminSetPostSpam(post._id, !post.isSpam);
      await loadPosts();
    } catch (err) {
  ...
  const openRoom = async (room) => {
    ...
    try {
      const msgs = await adminGetChatMessages(room._id);
  ...
  return (
    <div style={{ ...pageWrap, display: "flex" }}>
      <Sidebar active={active} onSelect={setActive} items={tabs} />
      ...
      <div style={{ flex: 1, padding: "2rem 2.5rem" }}>
        <div style={{ marginBottom: "2rem" }}>
          ...
          <p style={{ color: "#5a4d9e", fontWeight: 500, fontSize: "1rem" }}>
            Monitor and manage Locora
          </p>
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontWeight: 800,
                  fontSize: "0.85rem",
                  border: isActive ? "none" : "2px solid rgba(124,111,224,0.3)",
                  background: isActive
                    ? "linear-gradient(145deg, #a89cf7, #7c6fe0)"
                    : "linear-gradient(145deg, rgba(255,255,255,0.9), rgba(240,230,255,0.6))",
                  color: isActive ? "white" : "#7c6fe0",
                  cursor: "pointer",
                  fontFamily: "'Nunito', sans-serif",
                  boxShadow: isActive
        ...
        {active === "Overview" && stats && (
          ...
          >
            ...
            <GlassCard>
              ...
              >
                {stats.openPosts} open · {stats.resolvedPosts} resolved ·{" "}
                {stats.spamPosts} spam
              </p>
            </GlassCard>
            <GlassCard>
              <p
        ...
        {active === "Users" && (
          <GlassCard>
            <div style={{ overflowX: "auto" }}>
              ...
              >
                <thead>
                  <tr
                    style={{
                      textAlign: "left",
                      borderBottom: "1.5px solid rgba(124,111,224,0.15)",
                    }}
                  >
                    {[
                      "Name",
                ...
                <tbody>
                  {users.map((u) => (
                    <tr
                      key={u._id}
                      style={{
                        borderBottom: "1px solid rgba(124,111,224,0.08)",
                      }}
                    >
                      <td
                        style={{
                      ...
                      <td style={{ padding: "0.75rem", color: "#5a4d9e" }}>
                        {u.email || u.phone}
                      </td>
                      <td style={{ padding: "0.75rem", color: "#5a4d9e" }}>
                        {u.location || "—"}
              ...
              {users.length === 0 && (
                <p style={{ color: "#8b80c8", padding: "1rem 0" }}>
                  No users found.
                </p>
              )}
            </div>
        ...
        {active === "Posts" && (
          ...
          >
                ...
                <GlassCard key={p._id}>
                  ...
                  >
                    ...
                    >
                      <PillButton
                        variant="outline"
                        onClick={() => handleSpamToggle(p)}
                      >
                        {p.isSpam ? "Unmark Spam" : "Mark Spam"}
                      </PillButton>
                      <PillButton
                        variant="danger"
            ...
            {posts.length === 0 && (
              <p style={{ color: "#8b80c8" }}>No posts found.</p>
            )}
          </div>
        )}
          ...
          >
            {requests.map((r) => (
              <GlassCard key={r._id}>
                ...
                >
                  <span style={{ fontWeight: 800 }}>{r.requester?.name}</span> →
                  offered to help{" "}
            ...
            {requests.length === 0 && (
              <p style={{ color: "#8b80c8" }}>No help requests yet.</p>
            )}
          </div>
        )}
        ...
        {active === "Chats" && (
          ...
          >
            ...
            >
                  ...
                  <GlassCard
                    style={
                      selectedRoom?._id === room._id
                        ? {
                            border: "1.5px solid rgba(124,111,224,0.5)",
                            boxShadow: "0 8px 28px rgba(124,111,224,0.25)",
                          }
                        : {}
                    }
                  >
                    <p style={{ fontWeight: 700, color: "#2d1b69" }}>
                      {room.helpPost?.title || "Untitled"}
                    </p>
                    <p style={{ fontSize: "0.8rem", color: "#8b80c8" }}>
              ...
              {chats.length === 0 && (
                <p style={{ color: "#8b80c8" }}>No chat rooms yet.</p>
              )}
            </div>
                  ...
                  >
                    {roomMessages.map((m) => (
                      ...
                      >
                        ...
                        <p style={{ fontSize: "0.9rem", color: "#3d2c8d" }}>
                          {m.content}
                        </p>
                      </div>
                    ))}
